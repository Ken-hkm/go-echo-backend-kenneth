...
func ExpertiseHandler(c echo.Context) error {
	// Retrieve the "experience" collection from the database
	collection := db.GetCollection("expertise")
	...
	for cursor.Next(ctx) {
		...
		expertises = append(expertises, exp)
	}
	...
	response := utils.Response[[]models.Expertise]{
		Status:     "success",
